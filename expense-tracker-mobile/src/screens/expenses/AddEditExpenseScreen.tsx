...
import { ExpenseStackParamList, Transaction } from '../../types';
...
type Props = {
  navigation: NativeStackNavigationProp<ExpenseStackParamList, 'AddEditExpense'>;
  route: RouteProp<ExpenseStackParamList, 'AddEditExpense'>;
};
...
const AddEditExpenseScreen: React.FC<Props> = ({ navigation, route }) => {
  const existing = route.params?.transaction;
  const isEdit = !!existing;
  const userId = useAuthStore((s) => s.userId) ?? '';
  const queryClient = useQueryClient();
  const categories = useLocalStore((s) => s.categories).filter(
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <PickerModal
        ...
        items={CURRENCIES.map((c) => ({ id: c, name: c, icon: 'cash-outline', color: COLORS.primary }))}
