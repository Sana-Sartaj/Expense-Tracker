...
type Props = {
  navigation: NativeStackNavigationProp<ExpenseStackParamList, 'ExpenseList'>;
};
...
const ExpenseListScreen: React.FC<Props> = ({ navigation }) => {
  ...
  // Filter to EXPENSE type only and apply search
  const expenses = useMemo(() => {
    const filtered = all.filter((t) => t.transactionType === 'EXPENSE');
    ...
    <View style={styles.screen}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TransactionItem
            transaction={item}
            onEdit={() => navigation.navigate('AddEditExpense', { transaction: item })}
        ...
        ListEmptyComponent={
          <EmptyState
            ...
            onAction={
              search
                ? undefined
                : () => navigation.navigate('AddEditExpense', { transaction: undefined })
            }
          />
        }
      />
      ...
      <TouchableOpacity
        style={styles.fab}
        onPress={() => navigation.navigate('AddEditExpense', { transaction: undefined })}
      >
